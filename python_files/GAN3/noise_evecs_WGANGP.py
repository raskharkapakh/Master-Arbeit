...
def get_critic_model(latent_dim):
    ...
    x = ZeroPadding2D((2, 2))(evecs_input)
    x = conv_block(
        x,
        64,
        kernel_size=(5, 5),
...
def get_generator_model(latent_dim):
    ...
    x = Dense(np.prod(evecs_SHAPE), use_bias=False)(x)
    #x = BatchNormalization()(x)
    x = LeakyReLU(0.2)(x)
...
class noise_evecs_WGANGP(Model):
    ...
    def compile(self):
        super(noise_evecs_WGANGP, self).compile()
    ...
    def train_step(self, real_evecs):
        ...
        for i in range(self.c_steps):
            ...
            with tf.GradientTape() as tape:
                ...
                #print(f"fake_evecs0: {fake_evecs.shape}")
                # -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
                """
                # TODO: need to drop one last vec of evecs
                fake_shape = fake_evecs.shape
                fake_evecs = fake_evecs[:, :fake_shape[1]-1, :, :]
                print(f"fake_evecs: {fake_evecs.shape}")
                """
                # -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        with tf.GradientTape() as tape:
            ...
            # TODO:need to drop one last vec of evecs
            """
            generated_shape = generated_evecs.shape
            generated_evecs = generated_evecs[:, :generated_shape[1]-1, :, :]
            print(f"generated_evecs: {generated_evecs.shape}")
            """
            # -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    def generate_evecs(self, nb_trial=100):
        ...
        for i in range(nb_trial):
            ...
            if predictions[0, 0] > 0:
                print("real noise eigenvectors found")
                return True, norm_generated_evecs
                
        
        print("no real noise eigenvectors found")
