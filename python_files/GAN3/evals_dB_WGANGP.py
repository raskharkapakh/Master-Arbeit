...
from tensorflow.keras.layers import Input, Dense, Reshape, Multiply, Flatten, Dropout, BatchNormalization, Activation, ZeroPadding2D, LeakyReLU, UpSampling2D, Conv2D, Cropping2D
...
def get_critic_model():
    ...
    evals = Input(shape=EVALS_SHAPE)
    
    x = Flatten(input_shape=EVALS_SHAPE)(evals)
    x = Multiply()([x,-tf.ones(shape=(batch_size, np.prod(EVALS_SHAPE)))])
    x = Dense(512)(x)
    ...
    c_model = Model(evals, x, name="critic")
...
def get_generator_model():
    ...
    x = LeakyReLU(alpha=0.2)(x)
    #x = BatchNormalization(momentum=0.8)(x)
    x = Dense(np.prod(EVALS_SHAPE), activation='relu')(x)
    x = Multiply()([x,-tf.ones(shape=(batch_size, np.prod(EVALS_SHAPE)))])
...
class evals_dB_WGANGP(Model):
    def __init__(self):
        super(evals_dB_WGANGP, self).__init__()
    ...
    def compile(self):
        super(evals_dB_WGANGP, self).compile()
    ...
    def train_step(self, real_evals):
        ...
        for i in range(self.c_steps):
            ...
            with tf.GradientTape() as tape:
                ...
                fake_logits = self.critic(fake_evals, training=True)
                # Get the logits for the real eigenvalues
                real_logits = self.critic(real_evals, training=True)
                ...
                gp = self.gradient_penalty(batch_size, real_evals, fake_evals)
        ...
        with tf.GradientTape() as tape:
            ...
            gen_evals_logits = self.critic(generated_evals, training=True)
